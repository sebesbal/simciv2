...
__inline int power(int n, int k)
{
	int r=1;
...
//quo(a,n)*n+mod(a,n)=a
__inline int quo(int a, int n)
{
	return (a-mod(a,n))/n;
}

//signed mod
__inline int mods(int a, int n)
{
	int n1=absv(n), u=mod(a,n1);
	return 2*u>n1 ? u-n1 : u;
}

//rolls a by x*n close to u
//mod(r,n)==mod(a,n) && absv(r-u)<=n/2
__inline int modshift(int a, int u, int n)
{
	int g=a-u+n/2;
	int r=a-(g-mod(g,n));

	assc(mod(r-a,n)==0 && absv(r-u)<=n/2)
	return r;
}

//a*a/n
__inline int modq(int a, int n)
{
#ifdef using_vc
	__asm mov eax, a
	__asm imul a
...
//s : recursion eval-d by r
__inline void seq(int* s, int n, int* r, int f0=0, int f1=1, int m=0)
{
	int u=f0, v=f1, w;
	for (int k=0; k<n; k++)
	{
...
const double TW0PI=HALFPI*4.;

const double SQRT2=sqrt(2.);
const double SQRT3=sqrt(3.);
const double SQRT5=sqrt(5.);

const double ROOT3=.5*SQRT3;

const double GOLDR=.5*(sqrt(5.)+1.);

const double SQRT2PI=sqrt(TW0PI);
...
const double PH_DIRAC=PH_PLANCK/TW0PI;
...
__inline double radtogrd(double fi, double N=360.) { if (fi<0.) fi+=TW0PI; return fi/TW0PI*N; }

__inline double grdtorad(double fi, double N=360.) { return fi/N*TW0PI; }
...
public:
	randommoth(int seed) { reset(seed); }

	void reset(int seed)
	{
		u32 s=seed;
		int i;
		for (i=0; i<5; i++) { s=s*29943829-1; x[i]=s; }
	...
	unsigned int rndmod(unsigned int n)
	{
		assc(n>0)
		return random()%n;
	}
	...
	double rnd01()
	{
		const double g=1./double(1<<16);
		unsigned int r;
		double v=0.;
		r=random();
		v=(v+double(r&0xFFFF))*g;
		r>>=16;
		v=(v+double(r&0xFFFF))*g;
		r=random();
		v=(v+double(r&0xFFFF))*g;
	...
	double rndunit() { return 2.*rnd01()-1.; }
};
#endif

////
//obsolete, native windows generator of random numbers is lame
...
__inline double KLdta(double m1, double d1, double m2, double d2)
{
  ...
  return .5*(square(d1)+square(m1-m2))/square(d2)-.5-log(d1/d2);
}
...
{
	double fi=atan2(double(qy-py), double(qx-px)); if (fi<0.) fi+=TW0PI;
	int nfi=int(floor(fi/TW0PI*N+.5)); if (nfi>=N) nfi-=N;
	return nfi;
}

//hexa distance
__inline int dhx(int px, int py, int qx, int qy) { int dx=qx-px, dy=qy-py; return maxv(absv(dx),absv(dy),absv(dx-dy)); }
...
__inline double modf(double val, double period)
{
	...
	assc(val1>=0. && val1<period)
	return val1;
}

__inline double fitbyperiod(double val, double preval, double period=TW0PI)
{
	double delta=val-preval;
	double val1=val-floor((delta+.5*period)/period)*period;

	assc(fabs(val1-preval)<=.5*period)
	return val1;
}

//ex: distance between angles
__inline double moddelta(double a, double b, double p=TW0PI)
{
	double d=absv(a-b);
	d-=floor(d/p)*p;
	assc(d>=0 && d<p)
...
__inline void linsol2(double ax, double ay, double bx, double by, double px, double py, double& fa, double& fb)
{
	...
	//assc(fabs(ax*fa+bx*fb-px)<1e-8 && fabs(ay*fa+by*fb-py)<1e-8)
}
...
////
//2d affin trafo class

class aff2
{
public:
	double b[2], a[4];//a[0]=x'(x), a[1]=y'(x), etc (!! a[1] : first column, second row)

	aff2() { reset(); }
	aff2(double* p) { create(p); }

public:
	void reset() { b[0]=0.; b[1]=0.; a[0]=1.; a[1]=0.; a[2]=0.; a[3]=1.; }

	void create(double* p) { b[0]=p[0]; b[1]=p[1]; a[0]=p[2]; a[1]=p[3]; a[2]=p[4]; a[3]=p[5]; }

	double tx(double x, double y) { return b[0]+a[0]*x+a[2]*y; }

	double ty(double x, double y) { return b[1]+a[1]*x+a[3]*y; }

	double det() { return a[0]*a[3]-a[1]*a[2]; }

	double nsq() { return square(b[0])+square(b[1])+square(a[0]-1.)+square(a[1])+square(a[2])+square(a[3]-1.);  }

	void prod(aff2& u, aff2& v)
	{
		b[0]=u.b[0]+u.a[0]*v.b[0]+u.a[2]*v.b[1];
		b[1]=u.b[1]+u.a[1]*v.b[0]+u.a[3]*v.b[1];

		a[0]=u.a[0]*v.a[0]+u.a[2]*v.a[1];
		a[1]=u.a[1]*v.a[0]+u.a[3]*v.a[1];

		a[2]=u.a[0]*v.a[2]+u.a[2]*v.a[3];
		a[3]=u.a[1]*v.a[2]+u.a[3]*v.a[3];
	}

	void inv(aff2& u)
	{
		double d=u.det(); assc(absv(d)>1e-8)
		a[0]=u.a[3]/d; a[1]=-u.a[1]/d; a[2]=-u.a[2]/d; a[3]=u.a[0]/d;

		b[0]=-(a[0]*u.b[0]+a[2]*u.b[1]);
		b[1]=-(a[1]*u.b[0]+a[3]*u.b[1]);
	}

	//normal trafo
	void normal(aff2& u)
	{
		double d=det();	assc(absv(d)>1e-8)
		a[0]=u.a[3]/d; a[1]=-u.a[2]/d; a[2]=-u.a[1]/d; a[3]=u.a[0]/d;//inverse+transpose

		b[0]=0.; b[1]=0.;
	}

	//not exact, but acceptable
	void ratio(aff2& u, double r)
	{
		b[0]=r*u.b[0]; b[1]=r*u.b[1];
		a[0]=1.+r*(u.a[0]-1.); a[1]=r*u.a[1]; a[2]=r*u.a[2]; a[3]=1.+r*(u.a[3]-1.);
	}
...
//>convergence point
//hypo: x0=x+d, x1=x+a*d, x2=x+a*a*d, x is the fixpoint, a is the contraction factor
__inline double convpt(double x0, double x1, double x2, double& a)
{
	a=0.;
	double g=x2-2.*x1+x0;
	double h=x1-x0;
	if (absv(h)<1e-12 || absv(g)<1e-12) return INVALDD;//??
...
const int dir6x[6]={ 1,1,0,-1,-1, 0, };
const int dir6y[6]={ 0,1,1, 0,-1,-1, };

////

//neigbour-type

__inline int ng4(byte* pi, int pt)
{
	int w=0;
	if (pi[1]==0) w|=1;
	if (pi[pt]==0) w|=2;
...
__inline int ng8(byte* pi, int pt)
{
	int w=0;
	if (pi[1]==0) w|=1;
	if (pi[1+pt]==0) w|=2;
...
//255*6=1530 values, for drawing
__inline int hue2color(int h, int w=255)
{
	if (w<=0) return 0;

	int hm=mod(h,6*255);
	int hc=hm/255; hm-=hc*255;
...
//szurkeertek
__inline int gray(int w) { return ((w&0xFF)+(w>>8&0xFF)+(w>>16&0xFF)+1)/3; }
__inline int grayvalue(int w) { return ((w&0xFF)+(w>>8&0xFF)+(w>>16&0xFF)+1)/3; }//compatibility

__inline int red(int a) { return a&0xFF; }

__inline int green(int a) { return a>>8&0xFF; }

__inline int blue(int a) { return a>>16&0xFF; }
...
struct RGB { byte r,g,b,_; };

struct HSV { byte h,s,v,_; };

__inline void hsv2rgb(HSV hsv, RGB& rgb)
{
...
__inline void rgb2hsv(RGB rgb, HSV& hsv)
{
    byte rgbMin, rgbMax;
    ...
    hsv.v = rgbMax;
    if (hsv.v == 0) { hsv.h = 0; hsv.s = 0; return; }

    hsv.s = 255 * long(rgbMax - rgbMin) / hsv.v;
    if (hsv.s == 0) { hsv.h = 0; return; }
